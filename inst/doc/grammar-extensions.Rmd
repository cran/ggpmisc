...
| `ggplot2::geom_label()`        | `geom_label_npc()` | `character` vector          |
| `geom_linked_text()`           | _coming soon_      | `character` vector          |  
...
We can achieve this with `geom_linked_text()` together with a suitable position
...
A plot using `numeric` values for `time` and the default formatting of labels.
We can also pass other aesthetics recognised the `geom_text()` such as `angle`.
Here we also highlight and label the valleys.
...
We can obtain a very similar plot using `Date` for `time` but supplying a format
string. In addition marking both peaks and valleys.
...
Two new position functions supporting advanced nudging are made available by
package 'ggpmisc'. While `position_nudge()` implements shifts along $x$ and $y$
axes that are the same for every observation, the new functions compute the
direction of the nudging specifically for each observation.

One can think of this approach as nudging away from a focal point, a line or a
curve. It is deterministic, while repulsion is aleatory. Advanced nudging works
very well, but only for some patterns of observations and may require manual
adjustment of positions, repulsion is more generally applicable.
...
position. This is currently only supported by `geom_linked_text()`, but they
may be also supported by 'ggrepel' in the future.

### position_nudge_keep()

`position_nudge_keep()` (and `position_nudge_center()` With the default 
`direction = "none"`) behaves as `ggplot2::position_nudge()` but saves the 
original coordinates. Currently the saved original coordinates are used only
by `geom_linked_text()` as shown in the corresponding section above.
...
```
When using `geom_linked_text()` we need to change the default justification.
