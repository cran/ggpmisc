...
Satisitics `stat_peaks()` and `stat_valleys()` can be used to locate and annotate global and local maxima and minima in the variable mapped to the `y` aesthetic.
...
For the first example we use defaults to add an annotation with Pearson's corelation coefficient.
...
When weights are available, either suplied by the user, or computed as part of the fit, they are returned in `data`. Having weights available allows encoding them using colour. We here use a robust regression fit with `MASS::rlm`.
