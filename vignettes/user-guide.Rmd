...
`label` for the data frame containing the data for the table. The table is
created as a 'grid' grob and added as usual to the ggplot object. In contrast to
"standard" geoms, this geom by default does not inherit the globally mapped
aesthetics. With these approach in plots with facets the insets can be different
in each panel. It is also possible to inset more than one table in a single call
simply by creating a tibble with multiple rows.
...
Table themes are supported through parameter `table.theme` and if variables or constants are mapped to the 
`colour`, `fill`, `size`, `family`, aesthetics they override the corresponding
theme settings. The display of `rownames` and `colnames` can be enabled or
disable through parameter `table.rownames` and `table.colnames` and the 
horizontal justification of text in the core of the table through parameter
`table.hjust`.
...
R's 'grid' package defines several units that can be used to describe the locations
...
relative to the size of the grid viewport. The plotting area in a ggplot
is implemented as viewport and support for `"npc"` coordinates is relatively
easy to implement.
...
As is the case for `geom_text()` and `geom_text()` from package 'ggplot2',
...
for annotations, i.e., in those cases when we want to elements at specific
...
Two geometries are based on existing 'ggplot2' geometries. They are based on
`geom_text()` and `geom_label()`. We give an example using `geom_text_npc()` to
produce a "classic" labelling for facets matching the style of `theme_classic()`
and traditional scientific journals' design.
...
A ready formatted equation is also returned as a string that needs to be parsed into an _expression_ for display.
...
call `stat_poly_eq()` twice to be able to separately control the positions of
the two labels.
...
each tilde character, `~`, adding a rather small amount of white space. We shown
...
several labels instead of just two, and separate them various character strings. 
We also change the size of the text in the label.
...
the labels (as shown above) or use (as shown here) `atop()` within the expression to
...
mapping to both colour and fill aesthetics creates four groups, two with data in
...
Guide. Table themes and mapped aesthetic are supported.
...
Using defaults except for the colour aesthetic. Highlight 1/10 of observations 
...
is based on the colour aesthetic.
