...
As an example we produce plot where the inset plot is a zoomed-in detail from
...
`"top"`, `"bottom"`, `"left"`, `"right"` and `"centre"` (or its synonyms 
...
be applied to different data sets they help in keeping the code concise and
reusable.

As an example let's imagine that we want to add an institutional logo to a plot. 
...
the style of `theme_classic()` and traditional scientific journals' design.
...
expand the _y_ limits to ensure that no observations are occluded by the labels.
...
Next, one example how to remove the left hand side (_lhs_).
...
And a final example replacing both the  _lhs_ and the variable symbol used on the _rhs_.
...
each individual group. In the plot below the simultaneous mapping to both colour
...
but this default can be overridden as shown below.
