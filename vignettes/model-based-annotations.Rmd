...
A frequently used annotation in plots showing fitted lines is the display of the
parameters estimates from the model fit in an equation. `stat_quant_eq`
automates this for quantile regression of _y_ on _x_ fitted with function
`quantreg::rq`. This _statistic_ behaves similarly as `ggplot2::stat_quantiles`
with `method = "rq"`.

Please, see the section on `stat_poly_eq()` for applicable examples, as here
we highlight only the differences between these two statistics. Like
`stat_quantiles()`, `stat_quant_eq()` supports a `numeric` vector of quantiles
as argument and outputs as many equations per group as values are passed in 
this vector. The computed string `grp.label` contains by default a character
